...
def which_vgg(model: nn.Module) -> Tuple[int, str, bool]:
    nlayers = 0
    bn = False
    nclasses = model.classifier[6].out_features
    ...
    if nlayers == 38:
        # VGG19
        return nclasses, "E", bn
    elif nlayers == 32:
        # VGG16
        return nclasses, "D", bn
    elif nlayers == 26:
        # VGG13
        return nclasses, "B", bn
    elif nlayers == 22:
        # VGG11
        return nclasses, "A", bn
    raise ValueError('Unknown VGG configuration')

def register_activations(activations: List[Tensor]):
