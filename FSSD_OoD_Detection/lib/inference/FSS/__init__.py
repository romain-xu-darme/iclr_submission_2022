...
def compute_fss(model, n_layers, img_size, inp_channel):
    ...
    noise_imgs = torch.Tensor(noise_imgs).to(target)
...
def get_FSS_score_process(model, dataloader, fss, layer_index, magnitude=0, std=1):
    ...
    for data in dataloader:
        ...
        if magnitude != 0:
            ...
            if len(std) ==3:
                gradient.index_copy_(1, torch.LongTensor([0]).to(target), gradient.index_select(1, torch.LongTensor([0]).to(target)) / std[0])
                gradient.index_copy_(1, torch.LongTensor([1]).to(target), gradient.index_select(1, torch.LongTensor([1]).to(target)) / std[1])
                gradient.index_copy_(1, torch.LongTensor([2]).to(target), gradient.index_select(1, torch.LongTensor([2]).to(target)) / std[2])
            elif len(std) == 1:
                gradient.index_copy_(1, torch.LongTensor([0]).to(target), gradient.index_select(1, torch.LongTensor([0]).to(target)) / std[0])
